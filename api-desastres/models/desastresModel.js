// api-desastres/models/desastresModel.js
...
const DesastresModel = {
    ...
    // Obtener alertas activas de un estado
    ...
    // Obtener historial de desastres de un estado
    ...
    // Obtener todos los tipos de desastres
    getAllTiposDesastre: () => {
        return new Promise((resolve, reject) => {
        ...
        return new Promise((resolve, reject) => {
            const queries = {
                ...
                topDesastres: `SELECT t.nombre, COUNT(h.id) as total, t.icono
                              ...
                              GROUP BY t.id
                              ORDER BY total DESC`,
                alertasPorRegion: `SELECT e.region, COUNT(a.id) as alertas_activas
                                  FROM estados e
            ...
            db.all(queries.topEstados, [], (err, topEstados) => {
                ...
                db.all(queries.topDesastres, [], (err, topDesastres) => {
                    if (err) reject(err);
                    db.all(queries.alertasPorRegion, [], (err, alertasRegion) => {
                        if (err) reject(err);
